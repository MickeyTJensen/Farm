...
public class AnimalManager {
    ...
    public void AnimalMenu(Scanner scanner) {
        while (true) {
    ...
    public void feedAnimal(Animal selectedAnimal, Scanner scanner) {
        ...
        for (Crop crop : cropManager.getCrops()) {
            String cropType = crop.getType();
            if (selectedAnimal.acceptableCropType(crop)) {
                acceptableCropType = true;
                break; // No need to check further if we found at least one acceptable crop
            }
        }

        if (!acceptableCropType) {
            System.out.println("There is no acceptable crop type specified for this animal. Please add a new crop.");
            // You can add a prompt here to add a new crop or return to the main menu
            // For example: addNewCrop(scanner); // assuming you have a method for adding a new crop
            return;
        }
